...
class AdminManage:
    def __init__(self, parent_window):
        ...
        self.age = []
        # 打开数据库连接
        db = pymysql.connect(host='localhost', port=3306, db='student', user='root', password='')
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
...
class Changekey:
    def __init__(self, parent_window):
        ...
        label = tk.Label(self.window, text='修改学生', bg='green', font=('Verdana', 20), width=30, height=2)
        label.pack()
    ...
    def login(self):
        ...
        stu_pass = None
        # 数据库操作 查询管理员表
        db = pymysql.connect(host='localhost', port=3306, db='student', user='root', password='')  # 打开数据库连接   已经修正删除多余空格
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
...
class Adminlogin:
    def __init__(self, parent_window):
        ...
        label = tk.Label(self.window, text='Adminlogin', bg='green', font=('Verdana', 20), width=30, height=2)
        label.pack()
